...
if (empty($_POST) === false) {
    ...
    if (empty($errors) === true) {
        if (sha1($_POST['current_password']) === $employee_data['password']) {
...
if (isset($_GET['success']) && empty($_GET['success'])) {
    ...
    if (empty($_POST) === false && empty($errors) === true) {
        change_password_employee($_SESSION['employee_id'], $_POST['password']);
        header('Location: changepassword_employee.php?success');
    } else {
